...
public class ControllerExceptionHandler extends ResponseEntityExceptionHandler{
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
		...
		ErroResposta er = new ErroResposta (status.value(), "Campos inválidos foram inseridos, favor verificar", LocalDateTime.now(), null);
		return super.handleExceptionInternal(ex, er, headers, status, request);
	}
	
	@ExceptionHandler(EmailException.class) // exceção de E-mail
	protected ResponseEntity<Object> handleEmailException (EmailException e) {
		
		List<String> erros = new ArrayList<>();
		erros.add(e.getMessage());
		HttpStatus http = HttpStatus.BAD_REQUEST;
		ErroResposta er = new ErroResposta(http.value(), e.getLocalizedMessage(), LocalDateTime.now(), erros);
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(er);
	}
	
	@ExceptionHandler(ConfirmaSenhaException.class) // exceção de Senha
